...
def build_summary_short(row: pd.Series) -> str:
    ...
    desc = str(row["description"]).strip()[:120] if pd.notna(row["description"]) and row["description"] else "no description"
...
def build_summary_full(row: pd.Series) -> str:
    ...
    desc = str(row["description"]).strip() if pd.notna(row["description"]) and row["description"] else "No description provided."
